...
def _analyze_frame(frame) -> dict:
    """Simple face detection using OpenCV — no external model needed"""
    try:
...
def _calculate_confidence(emotions: dict) -> float:
    ...
    if positive + negative == 0:
        return 0.0
    return round(
        min(10, max(1, (positive / (positive + negative)) * 10)), 1)
        ...
        with self.lock:
            ...
            if not valid_conf:
                return {
                    ...
                    "dominant_emotion": "not detected",
                    "emotions": {},
                    "feedback": "Face not visible. Sit in front of camera.",
                }
